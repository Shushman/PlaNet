...
# TODO: Hack for discrete action spaces
# Proposal distribution assumes an independent multinomial for each time-step (eek) whose weights are upvoted by elite samples
class DiscreteMPCPlanner(jit.ScriptModule):
  __constants__ = ['action_size', 'planning_horizon', 'optimisation_iters', 'candidates', 'top_candidates', 'num_actions']

  def __init__(self, action_size, planning_horizon, optimisation_iters, candidates, top_candidates, transition_model, reward_model, num_actions):
    super().__init__()
    self.transition_model, self.reward_model = transition_model, reward_model
    self.action_size, self.num_actions = action_size, num_actions
    self.planning_horizon = planning_horizon
    self.optimisation_iters = optimisation_iters
    self.candidates, self.top_candidates = candidates, top_candidates
  ...
  def forward(self, belief, state):
    ...
    # Start with uniform weights for all actions (all 1s; don't need to be normalized)
    action_weights = torch.ones(self.planning_horizon, self.num_actions)

    for _ in range(self.optimisation_iters):

      # Sample candidate number of action sequences from action probs
      actions = torch.multinomial(action_weights, B*self.candidates, replacement=True) # TODO: This reshaping might be incorrect?
      ...
      # TODO: Check that the internal shaping makes sense
      # Update action_weights by adding according to best_actions values
      for r in range(action_weights.shape[0]):
        for v in best_actions[r, :]:
          action_weights[r, v] += 1
    
    # Return the highest weight of first action index 
    return action_weights[0,:].argmax()